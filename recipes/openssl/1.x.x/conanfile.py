...
"""
    ...
    def _run_make(self, targets=None, makefile=None, parallel=True):
        ...
        if self.settings.os == 'Android':
            # We don't want the version in the SONAME. qtandroiddeploy doesn't support libs with version suffix
            command.append("SHLIB_VERSION_NUMBER=")
            command.append("SHLIB_EXT=.so")
        if makefile:
